...
def fill_unvoiced_f0(f0: np.ndarray) -> np.ndarray:
    """
    Replace zeros with the last non-zero F0 value (simple forward fill).
    If there are no voiced frames at all, returns the original array.
    """
    f0 = f0.copy()
    nz = f0 > 0
    if not np.any(nz):
...
def get_loudness(
    ...
    freqs_safe = np.maximum(freqs, 1.0)  # avoid log10(0) warning
...
def extract_features(wav, sr=SR):
    ...
    raw_f0 = get_f0(y, sr=sr)               # shape [T]
    raw_f0 = np.asarray(raw_f0, dtype=np.float32)

    # voicing mask (1 where pyin thinks voiced, else 0)
    voicing = (~np.isnan(raw_f0)).astype(np.float32)

    # NaNs -> 0 for now
    f0 = np.nan_to_num(raw_f0, nan=0.0)
    # Fill unvoiced regions by forward fill
    f0_filled = fill_unvoiced_f0(f0)

    loudness = get_loudness(y, sr=sr)
    onset = get_onset(y, raw_f0, sr=sr)     # you can use raw_f0 or f0_filled
